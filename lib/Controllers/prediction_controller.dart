...
class PredictionController extends GetxController {
  ...
  // downloadMatching
  var downMatchingDataLoading = false.obs;
  Future<void> downloadMatchingData() async {
    downMatchingDataLoading(true);

    try {
      var response = await apiService.getRequestNoToken(
          endpoint:
              "api/matching/fetchedMatchingLastData/${dashController.userDetails["userProfileId"]}"
...
// ${BASE_URL}api/payments/webInvoice/${userProfileId}
      ...
      if (data != null) {
        ...
        // getAstroPayload['location'] = astroDataWebhook["location"];
        // getAstroPayload['username'] =
        //     "${astroDataWebhook["firstName"]} ${astroDataWebhook["lastName"]}";
        // "firstName": "SUGALI RAMA",
        //   "": "NAYAK",
        //"": "Bastipadu",
        ...
        //  downloadpostAstroData(finalPayload.value);
...
//     String zodiacKey = entry.key; // Get the Zodiac name
      ...
      //   var response = await apiService.postRequest(
      //     endpoint:
      //         "api/lifeTimeHoroscope/Save?userProfileId=${dashController.userDetails["userProfileId"]}",
      ...
      downMatchingDataLoading(false);
    }
  }

  ///

  // getastrodata for download
  Future<void> downloadHoroscopeData() async {
...
// ${BASE_URL}api/payments/webInvoice/${userProfileId}
      ...
      if (data != null) {
        ...
        finalPayload.value = {
          ...
          "pob": getAstroPayload['location'],
          "tz": getAstroPayload['Tz'],
          "style": getAstroPayload['Style'],
          "service": "lifetime",
          "userName": getAstroPayload['username'],
        };
...
//     String zodiacKey = entry.key; // Get the Zodiac name
  ...
  // downnloadpostAstrocard
  Future<void> downloadpostAstroData(Map payload) async {
    postastroDataLoading(true);
